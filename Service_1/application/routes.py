...
def home():
    ...
    response = requests.get('http://service-4:5003/colourpicker')
    
    colourpicked = response.text

    
    db.session.add(colours_picked( Picked = colourpicked ))
    db.session.commit()
    
    colourspicked = colours_picked.query.order_by(colours_picked.ColourID.desc()).limit(8).all()
